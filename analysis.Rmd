...
```{r}
viparc[356:386, ] %>% 
  dplyr::mutate(disease = make_outcome(., c("malaise", "diarrhoea"), 2)) %>% 
...
```{r}
viparc %>% 
  dplyr::mutate(amu = make_exposure(., c("colistin_use", "oxytetracycline_use"))) %>% 
...
```{r}
viparc %>% 
  dplyr::mutate(amu_first_weeks = first_weeks(., c("amoxicillin_use", "ampicillin_use"))) %>%
...
```{r}
viparc %>% 
  dplyr::mutate(amu_first_weeks = first_weeks(., c("amoxicillin_use", "ampicillin_use"), 2)) %>% 
  dplyr::select(farm, flock, week, amoxicillin_use, ampicillin_use, amu_first_weeks)
```
...
```{r}
viparc %>%
  dplyr::mutate(amu_observation = make_outcome(., c("amoxicillin_use", "ampicillin_use"), 2)) %>% 
...
viparc %>% 
  ...
  # (filtering has to be after make_exposure() and make_outcome() calls)
...
(tmp <- viparc %>% 
  dplyr::mutate(amu = make_exposure(., c("colistin_use", "oxytetracycline_use"))) %>% 
  dplyr::mutate(disease = make_outcome(., c("malaise", "diarrhoea"), 2)) %>% 
  dplyr::mutate(amu_first_weeks = first_weeks(., c("colistin_use", "oxytetracycline_use"))) %>% 
  dplyr::mutate(amu_observation = make_outcome(., c("colistin_use", "oxytetracycline_use"), 2)) %>% 
  dplyr::filter(prophylactic(., c("malaise", "diarrhoea"),
                             c("colistin_use", "oxytetracycline_use"), 2, 2)) %>% 
...
pipeline1 <- function(df, clinical_signs_filtering = NULL, antimicrobials_filtering = NULL, lcs,
  ...
  df %>% 
    ...
    dplyr::mutate(disease = make_outcome(., clinical_signs_output, watching)) %>% # creates disease variable
...
pipeline3 <- function(df, clinical_signs_filtering = NULL, antimicrobials_filtering = NULL, lcs,
  ...
  df %>% 
    ...
    dplyr::mutate(disease = make_outcome(., clinical_signs_output, watching)) %>% 
    dplyr::mutate(amu_first_weeks = first_weeks(., antimicrobials_week1, wk)) %>% 
    dplyr::mutate(amu_observation = make_outcome(., antimicrobials_watching, watching)) %>% 
    dplyr::filter(prophylactic(., clinical_signs_filtering, antimicrobials_filtering, lcs, lam)) %>% 
