...
* `filter_weeks(df, clinical_signs, antimicrobials, lcs, lam)`: performs the
...
function `filter_weeks()` that performs that filtering. This function makes use
...
It works as expected. Let's now define another utilitary function that adds a
vector of suffixes `suff` to a character vector `x` where we want all possible
...
`lagging()` and `add-suffix` functions:
...
# stops if there is a variable whose name ends with a digit (this is a
# requirement from the lagging() function):
  df %>% 
    names() %>% 
    grepl("\\d$", .) %>% 
    any() %>% 
...
# antimicrobials with lags 0n to lam:
  vsel <- purrr::map2(list(clinical_signs, antimicrobials), list(lcs, lam), add_suffix) %>% 
    unlist() %>% 
    c(clinical_signs)
...
# is a list of lam data frames.
    ...
    dplyr::mutate(include = dplyr::select(., !! vsel) %>% rowSums() %>% not()) %>% # include only the rows for which there is not missing values in the clinical signs and antimicrobials variables.
...
(Hence the 2 `stopifnot()` calls at the beginning of the function.) Indeed, this
is a requirement of the use of the `lagging()` function because this function
does add some digit at the end of the variables names and also adds some missing
values. For that reason, we need to make sure that the original data set does
not have these characteristics. Let's try it:
...
`filter_weeks()` function, returns the original data frame augmented with a
...
Seems to work as expected. Note again that, similarly to the `filter_weeks()`
...
Here we show how to assemble the functions `filter_weeks()`, `make_exposure()`,
...
viparc %>% 
  ...
  dplyr::filter(include) %>% # filtering has to after make_exposure() and make_outcome() calls
