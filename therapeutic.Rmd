...
rearrange_data <- function(gap, x) {
  prepare_data(viparc, gap = gap, x = x) %>% 
    filter(arm == "after") %>% 
    select(farm, flock, week, ends_with("_use")) %>% 
    tidyr::pivot_longer(ends_with("_use"), names_to = "antibiotics", values_to = "presence") %>% 
    mutate(antibiotics = with(ab_classes, setNames(class, antimicrobial))[antibiotics]) %>% 
    distinct()
}
```
...
by_week <- function(gap, x) {
  ...
  a <- tmp %>%
    ...
    tally() %>% 
    filter(! is.na(antibiotics)) %>% # because for some combinations of gap and x, the antibiotic may not be known
    pull(n)
  b <- tmp %>% 
    filter(presence) %>%
    select(-antibiotics) %>%
    group_by(farm, flock, week) %>%
    summarise(presence = sum(presence) > 0) %>%
    filter(presence) %>%
    nrow()
  c(a, b)
